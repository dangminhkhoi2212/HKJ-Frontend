...
export const get = async (query: TQuery<THireQuery>): Promise<THire[]> => {
	return (
...
export const getCount = async (query: TQuery<THireQuery>): Promise<number> => {
	return (
...
export const create = async (data: THire): Promise<THire> => {
	return (await interceptor.post("/hkj-hires", { ...data, isDeleted: false }))
		.data;
};
const hireService = {
	get,
