        ...
        return (
          <div className="inline-flex flex-col gap-1">
            {value?.map((v: string, i: number) => (
              <div key={i}>
                <Tag color={colorsTag[i % colorsTag.length]}>{v}</Tag>
              </div>
            ))}
          </div>
        );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex justify-between items-center">
        <div>
          <Button onClick={() => refresh()} icon={<RotateCcw size={18} />}>
            Làm mới
          </Button>
        </div>
        <p>
          Đã tìm thấy <span className="font-bold">{pagination.total}</span> tài
